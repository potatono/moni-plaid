...
def create_link_token():   
    try:
        user_id = ('user_id' in request.form and request.form['user_id'] or 
                   'nobody-'+str(time.time()))

        plaid_req = LinkTokenCreateRequest(
            products=products,
            client_name='Monility',
            country_codes=list(map(lambda x: CountryCode(x), PLAID_COUNTRY_CODES)),
            language='en',
            user=LinkTokenCreateRequestUser(
                client_user_id=user_id
            ),
            webhook=PLAID_REDIRECT_URI + "/api/transactions_webhook",
            redirect_uri=PLAID_REDIRECT_URI
        )
       
        plaid_res = client.link_token_create(plaid_req)

        return jsonify(plaid_res.to_dict())
...
def store_access_token():
    ...
    ref = (db.collection('tokens').document('services')
        .collection('plaid').document(item_id))

    ref.set(doc)
    return jsonify(doc)

# Fallback to the old API endpopint from quickstart for easy testing of this code without
# spinning up the iOS app.
@app.route('/api/set_access_token', methods=['POST'])
def set_access_token():    
    return store_access_token()

@app.route('/api/transactions_webhook', methods=['POST'])
def transactions_webhook():
    ...
    if data['webhook_type'] == 'TRANSACTIONS':
            ...
            ref = (db.collection('tokens').document('updates')        
                  .collection('plaid').document(item_id))
            
            doc = {
                "item_id": item_id,
                "updated_on": firestore.firestore.SERVER_TIMESTAMP
            }
